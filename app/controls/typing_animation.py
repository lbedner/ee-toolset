...
class TypingAnimation(ft.Row):
    def __init__(
        ...
        bubble_shape: Union[ft.BoxShape, Literal["circle", "rectangle"]] = ft.BoxShape(
            "circle"
        ),
        **kwargs
    ):
        ...
        assert (
            self.bubble_count > 1
        ), "The bubble_count must be greater than or equal to 2."
        ...
        class CircleBubble(ft.Container):
            ...
            def __init__(self, bubble_color: str, size=bubble_size, shape=bubble_shape):
                ...
                assert isinstance(
                    shape, (ft.BoxShape, str)
                ), "bubble_shape must be of type BoxShape or str"
                self.shape = (
                    ft.BoxShape(bubble_shape)
                    if isinstance(bubble_shape, str)
                    else bubble_shape
                )
